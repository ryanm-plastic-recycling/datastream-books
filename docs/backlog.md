...
> suggested target phase, and a priority field that is **empty by
> default** -- prioritization is a separate exercise.
>
> Compiled 2026-05-21 from the documentation audit at
> [`audits/audit-2026-05-21-evening.md`](audits/audit-2026-05-21-evening.md)
> §E. Updated as items land or new items are discovered.
>
> **How to use this doc:**
>
> - When picking up work, scan the relevant target-phase section.
...
>   phase section with the next BL-id (sequentially -- gaps OK).
...
Priority is left blank by default -- to be filled in via a separate prioritization exercise.
...
- **Description:** Decision §22 says "Vendors/customers added as needed" with ambiguous "as needed" semantics. Need an explicit decision (likely §67+ in the decision log) capturing whether vendors are ERP-synced or Books-owned for Phase 8 AP design.
...
## Phase 7A S4+ (gated by §66 reaffirmation)
...
- **Target phase:** Phase 7A S4 (gated by §66 reaffirmation).
- **Priority:** _(TBD)_
- **Status:** [Open]
...
- **Target phase:** Phase 7A.
- **Priority:** _(TBD)_
- **Status:** [Open]

### BL-47 -- (NEW per audit risk R-A-19) Form-level read-only fields on JE form

- **Description:** Make `rm_postedby_user`, `rm_journalentrynumber`, `rm_totaldebit`, `rm_totalcredit` read-only at the form level (maker portal property), not just at the plugin layer. Currently the maker portal accepts edits to these fields and the save throws -- confusing during Pam-facing demo.
- **Source:** Risk-register R-A-19 (added 2026-05-21). Audit Section G5.
- **Target phase:** Phase 7A S4 (must land before any Pam-facing shell demo of 7A artifacts).
- **Priority:** _(TBD -- but deadline is firm: before shell demo)_
- **Status:** [Open]
...
- **Target phase:** Any session (doc-only, low cost). Could even land in this cleanup batch.
- **Priority:** _(TBD)_
- **Status:** [Open]
...
- **Target phase:** Phase 7A S4 (recommended) or Phase 8 (latest -- before any further posts that would extend the chain).
- **Priority:** _(TBD)_
- **Status:** [Open]
...
- **Source:** `immutability-validation.md` "Still-pending follow-up checks".
- **Target phase:** Phase 7A S4 (recommended) or Phase 8.
- **Priority:** _(TBD)_
- **Status:** [Open]
...
|---|---|---|
| 2026-05-21 | Backlog file created from audit Section E. 46 items consolidated. Items BL-39 through BL-44 are doc-hygiene items completed during the same cleanup session. | Phase 7A audit cleanup (commit pending) |
