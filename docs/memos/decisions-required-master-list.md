...
>
> **How this gets used:** Reference this sheet before any Pam or executive meeting. Mark items `[Confirmed: §XX]` once decided. Items not yet decided remain in [`./executive-questionnaire.md`](./executive-questionnaire.md) and [`../backlog.md`](../backlog.md).
...
## Section 2 -- Decisions Pam Owns (Finance / Accounting)

Operational details of the finance system: workflows, field lists, approval rules, COA, reporting, period-close mechanics. Pam authorizes; IT supports.

| # | Decision | What we need from Pam | Unlocks |
|---|---|---|---|
...
| 6 | **Period close process** | Confirm month-end and year-end close steps as Pam runs them today. Who closes? What gets locked? What reports get pinned? | Period locks; close runbook; year-end close runbook |
...
Strategic scope, budget, timeline, external audit engagement, compliance. Exec authorizes; IT and Pam inform.
...
| 19 | **Project sponsorship + ownership confirmation** | Confirm President as sponsor, Pam as Finance System Owner, IT lead. Confirm escalation handling (reinforce ownership, not rescue). | Engagement model; CR triage authority |
...
|---|---|---|---|
| 21 | **Foreign currency** | Not in v1; schema future-proofed. | Pam |
| 22 | **Payroll** | Permanently out of scope (Paylocity stays). | Executive (one-time confirm) |
| 23 | **Data retention policy** | 7 years default for tax records and supporting documents. | Pam |
| 24 | **Physical document policy** | Going fully digital (SharePoint) post-cutover. | Pam |
| 25 | **Training approach** | TalentLMS + Scribe (existing pattern). | Pam |
...
- **Items 1, 2, 4, 5 + the Pam-consult on item 3** unlock Backend Track A and Phase 8 (AP + AR core) -- about 30% of remaining build
...
| 5 | Approval scope confirmations | Pam | [Open] |
| 6 | Period close process | Pam | [Open] |
| 7 | Bank list and ACH approach | Pam | [Open] |
| 8 | Reports that must exist at cutover | Pam | [Open] |
| 9 | Macola pain points to fix | Pam | [Open] |
| 10 | Macola archive scope | Pam | [Open] |
...
| 19 | Project sponsorship + ownership confirmation | Exec | [Confirmed: §30 -- Pam as Finance System Owner; exec confirmation pending] |
| 20 | Insurance/compliance review | Exec | [Open -- exec questionnaire §9] |
| 21 | Foreign currency | Pam | [Confirmed: §24 -- not in v1] |
| 22 | Payroll | Exec (one-time confirm) | [Confirmed: §6 -- permanently out of scope] |
| 23 | Data retention policy | Pam | [Pending: Pam] |
| 24 | Physical document policy | Pam | [Pending: Pam] |
| 25 | Training approach | Pam | [Confirmed: §27] |
