...
| 30 | 2026-05-19 | **Finance Lead role defined; person to be named by CFO (Fred) post executive working session** | **Mirrors Datastream ERP departmental owner pattern; CFO-down naming protects the role's authority** |
| 31 | 2026-05-19 | **ChangeRequest must support multi-image attachments** | **Concrete evidence beats verbal description** |
| 32 | 2026-05-19 | **President (Brandon) confirmed as Executive Sponsor; CFO (Fred) to introduce Finance Lead role to named individual following executive working session** | **Cascades ownership framing from top, via the CFO not IT** |
| 32a | 2026-05-22 | **CFO (Fred) owns the conversation that formally names the Finance Lead. Strategic Lead does not pre-position the role with the individual.** | **Ownership framing flows from CFO down; IT-side pre-positioning would dilute the role's authority before it lands** |
...
| 50 | 2026-05-20 | **Phase 7 JE entry: hybrid mode — Excel-like grid for power users, simpler form mode for clerks.** Toggle on the same page, not two separate screens. the Finance Lead's Macola muscle memory (target user) is Excel-style entry; clerks need form-mode guard rails. Both modes write to the same Dataverse entity through the same `PostJournalEntryPlugin` path. Hybrid is the single most complex Phase 7 screen and will be the long pole of Phase 7B. |
...
| 57 | 2026-05-20 | **Phase 7 Finance Lead validation model: NO initial design sign-off.** IT decides direction. The Finance Lead will exercise ownership via the in-app Change Request system after pages land in dev. **Why this works:** the Finance Lead is expected to react better in REACT mode than PLAN mode (validated through prior project history with the anticipated user); the CR system with multi-image attachments (decision §31) is the existing, working ownership mechanism. Compatible with the Finance Lead framing — owners describe pain on real artifacts, not on mockups. Risk: the Finance Lead may file heavy CR volume in the first weeks of Phase 7B; mitigated by the Phase 7E CR burn-down window. |
| 58 | 2026-05-20 | **Phase 7 front-end timing: STRICT SEQUENTIAL — front-end build does not begin until all backend phases (Phase 6B through the expected Phase 11/12+ backend items) are complete.** The Finance Lead first sees UI ~7 months from project start. Risks accepted explicitly: stakeholder confidence during the invisible period, late discovery of UX issues that would have surfaced earlier in a parallel build, late visibility into integration issues with the live ledger. Trade-off rationale: sequencing simplifies coordination (one front-end build against a stable backend), concentrates front-end attention when it happens, and avoids the cost of rebuilding UI against backend that is still in flux. |
...
| 66 | 2026-05-21 | **Phase 7A (UI Foundation) runs in parallel with Backend Track -- scope-bounded and provisional.** Supersedes the strict-sequential mandate of §58 *only* for documentation / research items in Phase 7A Sessions S1-S3: credential cleanup (S1), visual identity extraction (S2), sitemap design (S3). **No build work, no app module construction, no PCF authoring, no security role scaffolding, no Dataverse schema changes happen under this decision.** Any work past S3 (S4 app module + theme + logo packaging, S5+ component scaffolds, dashboard, PCFs) requires a **separate, explicit operator confirmation in a new session** -- it is not pre-approved here. Artifacts produced by S1-S3: [`../architecture/ui-styling.md`](../architecture/ui-styling.md) (CSS-variable-based minimal theme; status pill palette; logo asset plan), [`../architecture/ui-sitemap.md`](../architecture/ui-sitemap.md) (accounting-workflow-first sitemap; persona visibility deferred to 7B), and a follow-up note in [`../architecture/immutability-validation.md`](../architecture/immutability-validation.md) confirming the §45-era credential cleanup state is live. **R-7-01 risk profile shifts:** the originally-mandated "7+ month UI invisibility" interval reduces by however much shell-only demo material lands during the parallel-track window, which makes the *new* risk "shell-without-data Finance Lead-side dissatisfaction" rather than absence of UI. **Mitigation:** when the Finance Lead first encounters the Books app shell (currently planned for end of S4 in a subsequent session), the framing is explicit -- "shell only, no transactions yet; the pages you see will populate as backend phases land." This framing is mandatory in any demo conversation; it is not optional. **Why provisional:** the parallel-track override was made on-the-fly at Phase 7A kickoff to capitalize on a clean Phase 6B closure state. Committing now to the *full* Phase 7A build runtime would re-litigate §58 without daylight; deferring that decision keeps the option open. If, after S3 lands, the operator chooses not to continue parallel, S1-S3 artifacts remain valid (they describe state that will be needed whenever Phase 7A actually starts). |
| 67 | 2026-05-22 | **§66 reaffirmation gate resolved -- Phase 7A S4-S11 deferred; return to §58 strict sequential.** §66 made Phase 7A S1-S3 explicitly provisional and required a fresh operator confirmation before any work past S3 (S4 app module + theme + logo packaging, S5+ component scaffolds, dashboards, PCFs, security role scaffolding). The reaffirmation gate this morning resolved in favor of Option B from the handoff ([`../handoffs/handoff-2026-05-22-morning.md`](../handoffs/handoff-2026-05-22-morning.md) §G1) -- return to the strict-sequential ordering of §58 for all work past S3. **Rationale:** (a) Backend Track A (vendor / customer integration with ERP) is the current backend phase and is blocked on the [`../memos/executive-questionnaire.md`](../memos/executive-questionnaire.md) §17 vendor master scope question, which is Finance Lead-bound and unanswered; (b) UI scaffolding built before the vendor/customer schema settles risks rework -- the hybrid JE entry grid (§50), the multi-entity dashboard, and the AP/AR bill/invoice screens (§48) all touch vendor or customer lookups that depend on the §17 answer; (c) the R-A-19 form-level read-only mitigation that §66 required before any Finance Lead-facing shell demo remains a precondition for any future demo, so the §66 "shell-only demo" benefit was always gated behind that work even if S4 had landed; (d) the parallel-track context-switching cost between Backend Track A and UI shell work is real and was the original cost §58 sought to avoid. **What stays:** Phase 7A S1-S3 artifacts ([`../architecture/ui-styling.md`](../architecture/ui-styling.md), [`../architecture/ui-sitemap.md`](../architecture/ui-sitemap.md), the [`../architecture/immutability-validation.md`](../architecture/immutability-validation.md) follow-up confirming §45-era credential cleanup state) remain valid -- they describe state that will be needed whenever full Phase 7A actually starts. The 17 Phase 7 UX decisions (§46-§62) remain in force; they are not contingent on §66 or §67. The Phase 7A 11-session work breakdown remains valid as a planning artifact for when the phase resumes. **What does not happen under this decision:** no app module construction, no PCF authoring, no security role scaffolding, no Dataverse schema changes, no theme records, no logo packaging. The Phase 7A.5 candidate (R-A-19 mitigation as a small UI-flavored task that could unlock a Finance Lead demo regardless of S4-S11 status) is captured separately in [`../architecture/form-readonly-enforcement.md`](../architecture/form-readonly-enforcement.md) (drafted same session) and remains independently considerable. **Risk profile reset:** R-7-01 returns toward its original framing -- the parallel-track shortened-invisibility benefit of §66 only materializes through S3, which is a small fraction of total Phase 7 UI surface; the "shell-without-data Finance Lead dissatisfaction" sub-risk that §66 introduced is discharged because no shell beyond S1-S3 documentation will exist. R-7-05 likewise returns to its original framing: backend slippage compounds into 7B+ slip with no §66 parallel-track buffer past S3. The risk register entries for R-7-01 and R-7-05 already encoded the §66 reaffirmation as a conditional, so no separate amendment is required -- the conditional resolves to the "not reaffirmed" branch. **Revisit condition:** Phase 7A S4-S11 will be reconsidered once Backend Track A lands AND [`../memos/executive-questionnaire.md`](../memos/executive-questionnaire.md) §17 has a Finance Lead answer. Revisit is a fresh §-numbered decision, not an automatic resumption. **[Annotation 2026-05-22 per §68]:** in retrospect, the handoff §G1 Option C (continue S4 only, then defer S5-S11) had a stronger case than the morning reaffirmation gate credited. Option C would have landed S4 (app module + theme + logo packaging) -- a concrete artifact the Finance Lead could react to in the upcoming conversation -- while still honoring the §17-gated halt on S5+ work. The §68 principle that concrete artifacts produce better accounting-team feedback than design documents argues for Option C's visible-artifact bias over Option B's strict-sequential purity. §67 stands as written and is not reversed by this annotation; the annotation makes the trade-off honest in the record. Future reaffirmation gates should weight "concrete artifact for the accounting team" more heavily than today's call did, and should treat Option C-style hybrid paths as first-class candidates rather than splitting-the-difference compromises. |
| 68 | 2026-05-22 | **Operating principles for Claude Code sessions on this project codified in CLAUDE.md.** Process / engagement-model meta-decision, not architectural or technical. Captures five principles that govern how documentation-heavy and other routine work is sequenced and how concerns are surfaced: **(a) Progress is the default** -- when the operator says "move forward," execute; surface concerns once, briefly, then proceed; do not re-raise procedural concerns across turns. Real concerns (failed verification, ambiguous decision, unexpected file in a diff, scope expansion request) get surfaced; procedural concerns ("should I commit?", "ready to move to item 3?") do not. **(b) Concrete artifacts beat design documents for accounting-team feedback** -- the Finance Lead reacts better to something they can point at and complain about than to something they have to imagine. When the choice is between shipping 50% of a feature that is visible and 10% of a feature that is fully decided but invisible, prefer the visible 50%. Bias sequencing toward visible artifacts. **(c) Approve-in-batches for routine work; step-by-step for high-stakes.** Routine = doc updates, design docs, ranking passes, runbook updates, decision-log entries; complete the full pass, then surface a single end-of-session review. High-stakes work uses step-by-step approval with explicit per-step verification; the definition is: plugin C# code, SQL migrations (especially DENY grants, append-only constraints), production deploys to PRI-Books, schema changes touching posted-ledger tables, anything touching the hash chain, anything that could affect JE-2026-001005 or future audit-trail anchors, anything that would reverse or substantially modify a prior decision-log row. **(d) Scope discipline** -- surface adjacent finds in the end-of-session summary; do not silently expand scope; the operator decides whether to address now or add to the backlog. **(e) Operator-driven hours** -- when the operator wants to keep working, support that; do not push for stopping unless something concrete makes stopping the right call (failed verification, ambiguous decision, scope expansion). **Why now:** the morning execution session (commits `deb5e4e` through `0b5ea3a`) ran on per-step approval for every item; the operator concluded the pattern over-weights low-stakes work and slows progress on documentation-heavy passes. Codifying the distinction lets Claude Code recognize routine work and execute it as a batch without procedural friction. **What changes:** CLAUDE.md gains an "Operating Principles" section between "Communication Style" and "When to Stop and Ask"; the new principles supersede the implicit per-step approval pattern for routine work but leave the existing "When to Stop and Ask" guidance intact for high-stakes scenarios -- in fact "When to Stop and Ask" is reframed as the high-stakes subset of these principles. **What does not change:** the high-stakes list (plugin code, SQL migrations, prod deploys, schema, hash chain, JE-2026-001005, audit-trail anchors, prior decisions) continues to require step-by-step approval. AGENTS.md is not touched -- AGENTS.md is the universal default and remains the primary directive per CLAUDE.md "Primary Directive". The annotation appended to §67 records this principle's first retrospective application: the morning's §66 reaffirmation choice was made under the old per-step pattern and would have benefited from the new principle-(b) emphasis on concrete artifacts. |
| 69 | 2026-05-22 | **CI workflow async-import pattern adopted as the default for Dataverse solution imports.** Diagnostic story: four consecutive runs of the `deploy-dev` workflow (3 on 2026-05-21 evening, 1 retry on 2026-05-22 morning) reported solution import failures with the error signature "connection forcibly closed by remote host" at the ~5-minute mark. The Dataverse Solution History UI in PRI-Books-Dev showed each of the four "failed" runs had actually committed a successful import row whose timestamp matched the run's start window. **Root cause:** `pac solution import` in its default synchronous mode holds an HTTP connection to the Dataverse front-end gateway for the duration of the platform-side import. The gateway closes the connection at ~5 minutes regardless of whether the backend import is still in progress. The CLI exits with code 1 when the connection drops; the workflow step is marked failed. The platform-side import continues and completes successfully, leaving the workflow's reported status and the actual environment state out of sync. **Fix:** `.github/workflows/deploy-dev.yml` lines 188-198 (the "Import solution to PRI-Books-Dev" step) gained two flags on the `pac solution import` call: `--async` (submit as a server-side job, return a job ID, poll the job-progress endpoint until completion) and `--max-async-wait-time 30` (cap the polling window at 30 minutes; well above the 4-11 minute actual completion times observed). **Verification:** the post-fix run (commit `097da24`; GitHub run ID 26290727751, run #39) completed successfully on 2026-05-22 at 13:44:15Z. Total wall time 13m 35s. The import step itself took 10m 31s (13:33:33Z → 13:44:04Z) -- twice the ~5-minute sync-timeout threshold that broke the prior four runs, and well under the 30-minute async wait budget. Workflow conclusion = `success`. `DatastreamBooks` solution remains at version 1.0 Unmanaged in PRI-Books-Dev as expected. **Workflow design principle codified:** synchronous HTTP imports against Dataverse are unreliable for any solution import that crosses the gateway's ~5-minute connection-timeout threshold, which is a low bar for solutions of even modest size. **Async is the default pattern going forward for all `pac solution import` calls** in any workflow in this repo. The synchronous mode is retained only for genuinely fast imports (e.g., environment-variable-only solutions); any workflow added later that imports actual entity / plugin content should follow the deploy-dev.yml line 188-198 pattern. **What this does not retroactively fix:** the four "failed" workflow runs in GitHub Actions history (3 on 2026-05-21 evening, 1 on 2026-05-22 morning) remain marked failed in the GitHub UI for historical accuracy; their Dataverse-side outcome was already correct per the Solution History row pattern documented above, so no re-import is required. **What is intentionally out of scope:** path-filter triggering on the workflow (so runs fire only on solution / plugin / SQL changes) and solution-version auto-increment. Both tracked as separate concerns for a future workflow-hygiene session, not bundled into this fix. |
| 70 | 2026-05-22 | **Vendor and customer master ownership architecture: Books is the system of record for entity records; ERP receives a downstream projection of Books-mastered fields via plugin-driven push; ERP retains write authority on operations-only fields. Same record, two writers, field-level lanes -- not table-level read-only.** Books-mastered fields (read-only in ERP after sync): legal name, EIN, tax classification, W-9 status, 1099 reportable flag, banking / NACHA details, payment terms, hold-payment flag, credit terms (customers), approval status, AP / AR routing. ERP-mastered fields (writable in ERP only): site locations and shipping points, transportation routing preferences, operational approval flags for PO eligibility, operational notes, preferred-vendor flags by product, operational status. **Rationale:** vendor and customer records are fundamentally financial entities. The fraud surface (vendor bank info changes, ghost vendors, 1099 misclassification) sits in accounting and must be controlled there. W-9 collection and 1099 generation flow through AP. Cutover sequencing favors Books-as-master -- data migrates from Macola into Books cleanly, with no two-step migration. Operations' current usage (company name + site location tied to lot records) is read of Books-mastered fields + write of operations-mastered fields, unchanged in effective behavior. Supersedes the latent ambiguity in §22 ("Vendors/customers added as needed") for the system-of-record question; §22 stands for the "as-needed creation, no batch pre-load" intent. **Push pattern:** plugin on Books vendor / customer entity Update message fires a downstream push to ERP. Async, retry on failure, reconcile via verification job. Detailed plugin design is a Phase 8 (or earlier) scoping item -- not specified here. See backlog BL-52 (push plugin design), BL-53 (ERP-side write-permission lockdown), BL-54 (cutover-day reconciliation of existing ERP records with Macola-migrated Books data). **Who decided and how:** per §71 governance principle this is an IT architectural decision. The Finance Lead consults on field list and intake workflow design. COO concurs on operations impact. Confirmation expected in the upcoming exec rollout meeting. **What does not change for current operational use:** the two dual-role users currently authorized to add vendors / customers on the operations side shift their "add new" pattern to Books going forward; existing ERP vendor / customer records continue to function; the cutover-day reconciliation between Macola-migrated Books data and existing ERP records is a separate scoping item (BL-54). **Affected docs:** [`../memos/executive-questionnaire.md`](../memos/executive-questionnaire.md) §17 marked Resolved with citation here; [`../memos/decisions-required-master-list.md`](../memos/decisions-required-master-list.md) entry #3 restructured into the IT-decides column; [`../memos/pam-conversation-prep-2026-05-w22.md`](../memos/pam-conversation-prep-2026-05-w22.md) Item 1 rewritten from "decide who owns vendor records" to "field list, intake workflow, 1099 rules, approval routing for new vendor setup". |
| 71 | 2026-05-22 | **Governance principle: Architectural decisions sit with the Technical Strategic Lead (engineering + strategy); the Finance Lead consults; COO (Marco) concurs on cross-domain impact.** [Phrasing updated 2026-05-22 afternoon per §72 -- original lead read "IT decides architecture"; "IT" in this row's body and operational implications is read as "Technical Strategic Lead" going forward.] Architectural decisions about cross-system data flow, schema ownership, integration patterns, and system-of-record boundaries are IT decisions, not Finance Lead decisions. The Finance Lead owns the finance system and its operational details (workflows, field lists, approval rules, COA, reporting). The Finance Lead consults on architectural decisions that affect finance, but does not authorize them. Cross-domain architectural decisions (those affecting operations, sales, transportation, or other ERP-side concerns) additionally require COO concurrence before being locked in. This is not approval-by-veto from the COO; it is concurrence-by-information. IT proposes, COO confirms the operations impact is understood and manageable, decision proceeds. **Rationale:** §30 established the Finance Lead role, modeled on the Datastream ERP departmental ownership pattern. That ownership is real and unchanged. But ownership of a system does not extend to ownership of the architecture that connects systems. The Finance Lead role would be diluted if every cross-system question landed on the Finance Lead's plate; many such questions are not finance-domain questions even when finance touches them. **Retroactive scope clarification, not reversal:** this principle applies retroactively to §67 (Phase 7A sequencing -- IT decision with Finance Lead consult, not the Finance Lead's call), and prospectively to all future architectural decisions. Where a past decision implicitly attributed ownership to the Finance Lead that this principle reframes as IT + COO, the original decision is not reversed -- the framing is clarified going forward. §67's substantive outcome (return to §58 strict sequential) is not disturbed; only the framing of *who decided* is corrected. **Operational implications:** (a) future "decisions required" sheets categorize items as IT-decides / Finance Lead-decides / Exec-decides (see [`../memos/decisions-required-master-list.md`](../memos/decisions-required-master-list.md) restructured under this decision); (b) future Finance Lead-conversation prep docs frame architectural items as "here is what we are proposing; what operational details do you own?" (see [`../memos/pam-conversation-prep-2026-05-w22.md`](../memos/pam-conversation-prep-2026-05-w22.md) Item 1 rewrite); (c) future exec questionnaires separate strategic items (exec-decides) from architectural items (IT-decides with exec concurrence) from operational items (Finance Lead-decides). **What does not change:** §30 (the Finance Lead role) stands in full; the Ownership Artifacts table in the Finance Lead section continues to enumerate exactly which deliverables carry the Finance Lead's name (once named). The principle here clarifies what *isn't* in that table: cross-system architecture isn't there, and that absence is now load-bearing. |
| 72 | 2026-05-22 | **Technical Strategic Lead role definition.** Ryan M is designated the Technical Strategic Lead for Datastream Books. This role encompasses **engineering** (system design, build, deployment), **strategy** (architectural direction, technology selection, roadmap), **maintenance** (operations, troubleshooting, vendor management), and **architectural authority** per §71. This is not a traditional IT support role; it is a combined engineering-and-strategy function appropriate to the scale of this organization. **Read-this-as-that rule:** references to "IT" in earlier decisions (notably §71's body and operational implications, the master decisions sheet, and any cross-doc prose written before this row) should be read as "Technical Strategic Lead" going forward. The earlier text is not retroactively rewritten beyond the §71 lead sentence; this row carries the semantic load. **Reporting:** the President (Brandon) is the executive to whom this role reports for this project; Brandon is the Executive Sponsor per §32, and the CFO (Fred) names the Finance Lead per §30 but does not direct technical decisions. **Why this matters:** the organization is too small to support a traditional separation of "IT support" and "engineering management" and "strategy." Treating Ryan's role as just "IT" understates the architectural authority embedded in the role (per §71) and misframes how stakeholders should interpret cross-system decisions. The Technical Strategic Lead naming makes the combined engineering + strategy + maintenance scope explicit and clarifies why the role can authorize cross-system architecture without escalation. **Concurrence pattern:** decisions made under §71 architectural authority by the Technical Strategic Lead are logged in [`./concurrence-log.md`](./concurrence-log.md) (new this session) with a 5-business-day stakeholder concern window; this gives the Finance Lead, COO (Marco), and President (Brandon) visibility and an opportunity to raise concerns before the decision locks. The concurrence log is structural, not authority-shifting -- the Technical Strategic Lead still decides; stakeholders raise concerns through a defined channel rather than ad-hoc. |
...
## Finance Lead — Role Definition

> **Target operating model — not current state.** The Finance Lead has not yet been named. The CFO (Fred) will formally introduce the role to the named individual following the executive working session. Until that conversation happens, "Finance Lead" is a role definition, not a person. Strategic Lead does not pre-position the role with any candidate.

### Critical Framing: The Finance Lead OWNS This System
...
| **Datastream Books — Finance** | **Finance Lead (TBD — named by CFO post executive working session)** |

**The Finance Lead will be the business owner of the finance system. IT builds the system for them. They are responsible for whether it works.**
...
When the Finance Lead escalates to the President (Brandon) as Executive Sponsor, Brandon must reinforce ownership: "You own this. What do you need from IT to make it work?" — not "IT, why isn't this working for the Finance Lead?"

This expectation is set in the executive working session, before the CFO (Fred) names the Finance Lead.

### Ownership Artifacts (Finance Lead's Name On Them, Once Named)

| Deliverable | Finance Lead's Role |
|---|---|
...
### Ownership Language (Post-Naming)

- "The Finance Lead's accounting system" — not "the new accounting system"
- "What does the Finance Lead need it to do?" — not "what do we think they need?"
- In meetings: "When do you want to deploy this?" addressed to the Finance Lead — not "we plan to deploy this when..."

### Visible Ownership (Post-Naming)

- Finance Lead role identified in repo README
- Finance Lead identified on the executive memo as project co-lead (after naming)
- Finance Lead's contact in the user-facing app footer as the "Report an Issue" contact
- Finance Lead mentioned by role in leadership status updates as the owner

### Engagement Structure (Target Operating Model)
...
**The Finance Lead will triage and prioritize** finance-side CRs themselves. They close them out when satisfied. This is part of ownership.
...
Before kickoff, IT meets with the President (Brandon) as Executive Sponsor, the COO (Marco), and the CFO (Fred) — Wednesday 10 AM, Innovation Conference Room — to:

1. Confirm project leads: Executive Sponsor (Brandon), Strategic Lead (Ryan M), Finance Lead (role to be named by Fred following this session)
2. Work the decisions spreadsheet
3. Identify Finance Lead follow-ups so Fred can frame the role correctly when he introduces it
4. Confirm leadership response to escalations: reinforce ownership, not rescue
5. Confirm next steps, budget envelope, and timeline impact

The CFO (Fred) introducing the Finance Lead role to the named individual is critical — not IT. CFO-down endorsement lands differently than a tap from IT and protects the role's authority from day one.

### Failure Mode to Watch

If the Finance Lead, once named, cannot or will not accept ownership — insisting on critic role rather than owner — this is a serious early signal. Surface to the CFO (Fred) within 30 days of naming, not month 6.
...
2. Raise with the CFO (Fred) and the President (Brandon) as a process risk, not a personality complaint
...
|---|---|---|
| Bills | Amount > $X (TBD by Finance Lead) | Direct supervisor / designated approver |
| Journal entries | Amount > $Y (TBD by Finance Lead) | Second reviewer (≠ creator) |
...
| Write-offs | Amount > $Z (TBD by Finance Lead) | Controller |
...
1. **Executive working session:** President (Brandon), COO (Marco), CFO (Fred), and Strategic Lead — confirm ownership framing, Finance Lead role definition, escalation handling
2. **CFO (Fred) introduces Finance Lead role to named individual** (not IT, not Strategic Lead) — naming happens after the executive working session
3. **COA Pre-Population:** IT proposes draft; Finance Lead reviews and signs off (once named)
4. **Vendor Master Strategy:** "Add as needed" — Finance Lead reviews approach (once named)
5. **Entity Documentation:** Finance Lead coordinates with executive team (once named)
6. **Approval Threshold Definition:** Finance Lead sets dollar amounts (once named)
7. **Macola Archive Plan:** Retention duration and access pattern
8. **Finance Lead engagement cadence:** Weekly check-ins begin (once named)
...
| **Finance Lead, once named, does not accept ownership role** | **High** | **CFO (Fred) introduces role; surface to CFO within 30 days of naming if resistance appears. Not a project issue if persistent — becomes HR.** |
| **Leadership rescues Finance Lead from ownership during escalations** | **High** | **Mitigated by escalation protocol set in executive working session (Brandon, Marco, Fred + Strategic Lead) before Finance Lead is named** |
...
| Phase 1: Design sprint + repo setup + CI/CD + Finance Lead onboarding | 4–6 weeks | Data model, security, controls, ALM pipeline |
...
- **Finance Lead** — Business owner of the finance system; named by the CFO (Fred) post executive working session
- **Strategic Lead** — Combined engineering + strategy + maintenance + architectural authority for Datastream Books (Ryan M)
- **President (Brandon)** — Executive Sponsor
- **CFO (Fred)** — Finance executive; names the Finance Lead
- **COO (Marco)** — Operations executive; concurs on cross-domain architectural impact
...
- **SME** — Subject Matter Expert (note: the Finance Lead role is an OWNER role, not an SME role)
...
| 2026-05-19 | PRI-Books-Dev sandbox added; Finance Lead engagement protocol established |
| 2026-05-19 | **PRI-Books-Dev authenticated. Finance Lead role reframed from SME to OWNER (mirrors ERP pattern). ChangeRequest design includes multi-image attachment support. Executive working session scheduled with President (Brandon), COO (Marco), and CFO (Fred). President (Brandon) confirmed as Executive Sponsor. Strategy phase complete.** |
...
| 2026-05-22 | **CI workflow async-import pattern adopted (decision §69).** Four consecutive deploy-dev runs had reported import failures while Dataverse Solution History showed actual success -- root cause was pac CLI's synchronous import connection dropping at the Dataverse gateway's ~5-min timeout. Fixed by switching to `--async --max-async-wait-time 30` on the import call. Post-fix run (097da24, GitHub run #39) completed successfully in 13m 35s with the import step taking 10m 31s. | Workflow fix session (commit pending) |
| 2026-05-22 | **Vendor / customer master ownership architecture captured (decision §70).** Books is system of record; ERP receives a Books-mastered field projection via plugin-driven push; ERP retains write authority on operations-only fields (site locations, transportation, operational flags). Same record, two writers, field-level lanes. Resolves the §22 latent ambiguity for the system-of-record question and the [`../memos/executive-questionnaire.md`](../memos/executive-questionnaire.md) §17 vendor master scope question. Push plugin design, ERP write-permission lockdown, and cutover-day reconciliation captured as backlog items BL-52, BL-53, BL-54 in [`../backlog.md`](../backlog.md). |
| 2026-05-22 | **Governance principle codified (decision §71): IT decides architecture; the Finance Lead consults; COO (Marco) concurs on cross-domain impact.** Retroactively reframes §67 as an IT decision with Finance Lead consult (not the Finance Lead's call); §67's substantive outcome is not disturbed. Prospectively governs how future cross-system architectural decisions are categorized in the "decisions required" sheet and how Finance Lead-conversation prep docs frame architectural items. §30 (the Finance Lead role) stands unchanged. **[2026-05-22 afternoon update per §72: §71's lead sentence rephrased to "Architectural decisions sit with the Technical Strategic Lead (engineering + strategy); the Finance Lead consults; COO concurs on cross-domain impact." References to "IT" in §71's body and operational implications are read as "Technical Strategic Lead" going forward; no other retroactive edits to prior decisions.]** |
| 2026-05-22 | **Terminology and ownership-sequencing pass.** Repo-wide terminology cleanup: individual-name references to the prospective Finance Lead replaced with the role title; the prior System-Owner role label consolidated to "Finance Lead"; Executive Sponsor anchored as President (Brandon); CFO (Fred) added on first mention per document with subsequent "Fred"; COO (Marco) added on first mention per document with subsequent "Marco"; "Technical Lead" references consolidated to "Strategic Lead" (Strategic Lead is the operational shorthand for the Technical Strategic Lead role defined in §72). **Ownership-sequencing reversal:** pre-naming an individual to the Finance Lead role in writing was incorrect framing. Correct sequence: (1) executive working session (Brandon, Marco, Fred + Strategic Lead) happens first; (2) CFO (Fred) introduces Finance Lead role to the named individual AFTER the working session; (3) ownership framing flows from CFO down, not from IT across; (4) no individual is named to the Finance Lead role in any forward-looking document until Fred has formally named them. Decision row §30 reworded to remove pre-naming; §32 reworded to designate Brandon as Executive Sponsor and Fred as the role-namer for the Finance Lead; new §32a added: "CFO (Fred) owns the conversation that formally names the Finance Lead. Strategic Lead does not pre-position the role with the individual." Risks table reworded to remove pre-naming and "leadership rescues" individual variants; replaced with Finance-Lead-once-named variants. President memo preserved as sent (historical document); all other docs updated. |
| 2026-05-22 | **Technical Strategic Lead role defined (decision §72).** Ryan M designated Technical Strategic Lead for Datastream Books -- a combined engineering + strategy + maintenance + architectural-authority role appropriate to the scale of this organization, distinct from a traditional IT support role. References to "IT" in earlier decisions read as "Technical Strategic Lead" going forward (read-this-as-that rule; no retroactive rewrites). President (Brandon) is the executive to whom this role reports; Brandon sponsors per §32 (Executive Sponsor) and CFO (Fred) names the Finance Lead per §30, but neither directs technical decisions. Companion artifact created this session: [`./concurrence-log.md`](./concurrence-log.md) -- 5-business-day stakeholder concern window for decisions made under §71 authority. §70, §71, §72 each opened with windows closing 2026-05-29. |
